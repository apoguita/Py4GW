...
class BT:
    ...
    class Player:
        ...
        def InteractAgent(agent_id: int, log: bool = False):
            ...
            tree = BehaviorTree.ActionNode(
                name="InteractAgent", action_fn=lambda: _interact_agent(agent_id), aftercast_ms=100
            )
            ...
            tree = BehaviorTree.SequenceNode(
                children=[
                    BehaviorTree.ActionNode(
                        name="GetTargetID", action_fn=lambda node: _get_target_id(node), aftercast_ms=0
                    ),
                    # SubtreeNode factory receives *its own node* (with blackboard)
                    BehaviorTree.SubtreeNode(
                        name="InteractAgent",
                        subtree_fn=lambda node: BT.Player.InteractAgent(node.blackboard["target_id"], log=log),
                    ),
                ]
            )
        ...
        def ChangeTarget(agent_id, log: bool = False):
            ...
            def _change_target():
                if agent_id != 0:
                    ...
                    ConsoleLog(
                        "ChangeTarget", f"Changed target to agent {agent_id}.", Console.MessageType.Info, log=log
                    )
                    return BehaviorTree.NodeState.SUCCESS

                ConsoleLog(
                    "ChangeTarget", "Invalid agent ID provided for targeting.", Console.MessageType.Error, log=log
                )
                return BehaviorTree.NodeState.FAILURE
        ...
        def SetTitle(title_id: int, log: bool = False):
            ...
            def _set_title(title_id: int):
                ...
                ConsoleLog(
                    "SetTitle",
                    f"Set title to {TITLE_NAME.get(title_id, 'Invalid')}.",
                    Console.MessageType.Info,
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
        ...
        def SendChatMessage(channel: str, message: str, log=False):
            ...
            def _send_chat_message(channel: str, message: str):
                ...
                ConsoleLog(
                    "SendChatMessage", f"Sent chat message to {channel}: {message}.", Console.MessageType.Info, log=log
                )
                return BehaviorTree.NodeState.SUCCESS
    ...
    class Skills:
        ...
        def LoadSkillbar(template: str, log: bool = False):
            ...
            tree = BehaviorTree.ActionNode(
                name="LoadSkillbar", action_fn=lambda: _load_skillbar(template), aftercast_ms=500
            )
            return BehaviorTree(tree)
        ...
        def LoadHeroSkillbar(hero_index: int, template: str, log: bool = False):
            ...
            def _load_hero_skillbar(hero_index: int, template: str):
                ...
                ConsoleLog(
                    "LoadHeroSkillbar",
                    f"Loaded hero {hero_index} skillbar template.",
                    Console.MessageType.Info,
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
            ...
            tree = BehaviorTree.ActionNode(
                name="LoadHeroSkillbar", action_fn=lambda: _load_hero_skillbar(hero_index, template), aftercast_ms=500
            )
            return BehaviorTree(tree)
        ...
        def SetHardMode(hard_mode=True, log=False):
            ...
            def check_mode_and_log():
                if GLOBAL_CACHE.Party.IsHardMode() == hard_mode:
                    ConsoleLog(
                        "SetHardMode",
                        f"Mode set to {'hard_mode' if hard_mode else 'normal_mode'}.",
                        Console.MessageType.Info,
                        log=log,
                    )
                    return True
                ConsoleLog(
                    "SetHardMode", f"Failed to set hard mode to {hard_mode}.", Console.MessageType.Error, log=log
                )
                return False
            ...
            tree = BehaviorTree.SelectorNode(
                children=[
                    ...
                    BehaviorTree.SequenceNode(
                        ...
                        children=[
                            BehaviorTree.ActionNode(
                                name="TravelAction", action_fn=lambda: travel_action(outpost_id), aftercast_ms=3000
                            ),
                            BehaviorTree.WaitNode(
                                name="MapArrival", check_fn=lambda: map_arrival(outpost_id), timeout_ms=timeout
                            ),
        ...
        def TravelToRegion(outpost_id, region, district, language=0, log: bool = False, timeout: int = 10000):
            ...
            def arrived_early() -> bool:
                if (
                    ...
                    and Map.GetRegion() == region
                    and Map.GetDistrict() == district
                    and Map.GetLanguage() == language
                ):

                    ConsoleLog("TravelToRegion", f"Already at {Map.GetMapName(outpost_id)}", log=log)
                    return True
            ...
            def travel_action() -> BehaviorTree.NodeState:
                ConsoleLog("TravelToRegion", f"Travelling to {Map.GetMapName(outpost_id)}", log=log)
            ...
            def map_arrival() -> BehaviorTree.NodeState:
                if (
                    ...
                    and Map.GetMapID() == outpost_id
                    and Map.GetRegion() == region
                    and Map.GetDistrict() == district
                    and Map.GetLanguage() == language
                ):

                    ConsoleLog("TravelToRegion", f"Arrived at {Map.GetMapName(outpost_id)}", log=log)
                    return BehaviorTree.NodeState.SUCCESS
                ...
                children=[
                    ...
                    BehaviorTree.RepeaterUntilSuccessNode(
                        ...
                        child=BehaviorTree.SelectorNode(
                            ...
                            children=[
                                BehaviorTree.ConditionNode(
                                    name="CheckItemNameReady", condition_fn=_check_item_name_ready
                                ),
                                BehaviorTree.SequenceNode(
    ...
    class Agents:
        ...
        def GetAgentIDByName(agent_name: str) -> BehaviorTree:
            ...
            tree = BehaviorTree.SequenceNode(
                ...
                children=[
                    ...
                    BehaviorTree.RepeaterUntilSuccessNode(
                        name="WaitUntilAllNamesReadyRepeater",
                        timeout_ms=2000,
                        child=BehaviorTree.SelectorNode(
                            name="WaitUntilAllNamesReadySelector",
                            children=[
        ...
        def GetAgentIDByModelID(model_id: int, log: bool = False) -> BehaviorTree:
            ...
            def _search_model_id(node):
                ...
                if found != 0:
                    ConsoleLog(
                        "GetAgentIDByModelID",
                        f"Found agent ID {found} for model ID {model_id}.",
                        Console.MessageType.Info,
                        log=log,
                    )
                    ...
                    ConsoleLog(
                        "GetAgentIDByModelID",
                        f"No agent found for model ID {model_id}.",
        ...
        def TargetAgentByName(agent_name: str, log: bool = False):
            ...
            tree = BehaviorTree.SequenceNode(
                ...
                children=[
                    BehaviorTree.SubtreeNode(
                        name="GetAgentIDByNameSubtree", subtree_fn=lambda node: BT.Agents.GetAgentIDByName(agent_name)
                    ),
                    BehaviorTree.SubtreeNode(
        ...
        def TargetNearestNPC(distance: float = 4500.0, log: bool = False):
            ...
            def _find_nearest_npc(node):
                ...
                if nearest_npc != 0:
                    ConsoleLog(
                        "TargetNearestNPC",
                        f"Found nearest NPC with ID {nearest_npc} within distance {distance}.",
                        Console.MessageType.Info,
                        log=log,
                    )
                    return BehaviorTree.NodeState.SUCCESS
                ConsoleLog(
                    "TargetNearestNPC",
                    f"No NPC found within distance {distance}.",
                    Console.MessageType.Warning,
                    log=log,
                )
                return BehaviorTree.NodeState.FAILURE
            ...
            tree = BehaviorTree.SequenceNode(
                ...
                children=[
                    ...
                    BehaviorTree.SubtreeNode(
                        name="ChangeTargetSubtree",
                        subtree_fn=lambda node: BT.Player.ChangeTarget(
                            node.blackboard.get("nearest_npc_id", 0), log=log
                        ),
                    ),
                ],
            )
        ...
        def TargetNearestNPCXY(x, y, distance, log: bool = False):
            ...
            def _find_nearest_npc_xy(node):
                ...
                ConsoleLog(
                    "TargetNearestNPCXY",
                    f"No NPC found near ({x}, {y}) within distance {distance}.",
                    Console.MessageType.Warning,
                    log=log,
                )
                return BehaviorTree.NodeState.FAILURE
        ...
        def TargetNearestGadgetXY(x, y, distance, log: bool = False):
            ...
            def _find_nearest_gadget_xy(node):
                ...
                ConsoleLog(
                    "TargetNearestGadgetXY",
                    f"No gadget found near ({x}, {y}) within distance {distance}.",
                    Console.MessageType.Warning,
                    log=log,
                )
                return BehaviorTree.NodeState.FAILURE
            ...
            tree = BehaviorTree.SequenceNode(
                ...
                children=[
                    ...
                    BehaviorTree.SubtreeNode(
                        name="ChangeTargetSubtree",
                        subtree_fn=lambda node: BT.Player.ChangeTarget(
                            node.blackboard.get("nearest_gadget_id", 0), log=log
                        ),
                    ),
                ],
            )
        ...
        def TargetNearestItemXY(x, y, distance, log: bool = False):
            ...
            def _find_nearest_item_xy(node):
                ...
                ConsoleLog(
                    "TargetNearestItemXY",
                    f"No item found near ({x}, {y}) within distance {distance}.",
                    Console.MessageType.Warning,
                    log=log,
                )
                return BehaviorTree.NodeState.FAILURE
            ...
            tree = BehaviorTree.SequenceNode(
                ...
                children=[
                    ...
                    BehaviorTree.SubtreeNode(
                        name="ChangeTargetSubtree",
                        subtree_fn=lambda node: BT.Player.ChangeTarget(
                            node.blackboard.get("nearest_item_id", 0), log=log
                        ),
                    ),
                ],
            )
        ...
        def TargetNearestEnemy(distance, log: bool = False):
            ...
            def _find_nearest_enemy(node):
                ...
                ConsoleLog(
                    "TargetNearestEnemy",
                    f"No enemy found within distance {distance}.",
                    Console.MessageType.Warning,
                    log=log,
                )
                return BehaviorTree.NodeState.FAILURE
            ...
            tree = BehaviorTree.SequenceNode(
                ...
                children=[
                    ...
                    BehaviorTree.SubtreeNode(
                        name="ChangeTargetSubtree",
                        subtree_fn=lambda node: BT.Player.ChangeTarget(
                            node.blackboard.get("nearest_enemy_id", 0), log=log
                        ),
                    ),
                ],
            )
        ...
        def TargetNearestItem(distance, log: bool = False):
            ...
            def _find_nearest_item(node):
                ...
                ConsoleLog(
                    "TargetNearestItem",
                    f"No item found within distance {distance}.",
                    Console.MessageType.Warning,
                    log=log,
                )
                return BehaviorTree.NodeState.FAILURE
        ...
        def TargetNearestChest(distance, log: bool = False):
            ...
            def _find_nearest_chest(node):
                ...
                ConsoleLog(
                    "TargetNearestChest",
                    f"No chest found within distance {distance}.",
                    Console.MessageType.Warning,
                    log=log,
                )
                return BehaviorTree.NodeState.FAILURE
            ...
            tree = BehaviorTree.SequenceNode(
                ...
                children=[
                    ...
                    BehaviorTree.SubtreeNode(
                        name="ChangeTargetSubtree",
                        subtree_fn=lambda node: BT.Player.ChangeTarget(
                            node.blackboard.get("nearest_chest_id", 0), log=log
                        ),
                    ),
                ],
            )
