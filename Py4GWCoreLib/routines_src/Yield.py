...
class Yield:
        ...
        def FollowPath(
            ...
            for idx, (target_x, target_y) in enumerate(path_points):
                ...
                if not Checks.Map.MapValid():
                    ConsoleLog(
                        "FollowPath", "Map invalid before starting point, aborting.", Console.MessageType.Error, log=log
                    )
                ...
                while True:
                    ...
                    if not Checks.Map.MapValid():
                        ConsoleLog(
                            "FollowPath",
                            "Map became invalid mid-run, aborting movement.",
                            Console.MessageType.Warning,
                            log=log,
                        )
                    ...
                    if custom_exit_condition():
                        ConsoleLog(
                            "FollowPath",
                            "Custom exit condition met, stopping movement.",
                            Console.MessageType.Info,
                            log=log,
                        )
                        return False
    ...
    class Items:
        ...
        @staticmethod
        def CraftItem(
            output_model_id: int, cost: int, trade_model_ids: list[int], quantity_list: list[int]
    ...
    class Keybinds:
        ...
        def OpenSaveEquipmentTemplate(log=False):
            yield from Yield.Keybinds.PressKeybind(
                ControlAction.ControlAction_OpenSaveEquipmentTemplate.value, 75, log=log
            )
        ...
        def OpenSkillsAndAttributes(log=False):
            yield from Yield.Keybinds.PressKeybind(
                ControlAction.ControlAction_OpenSkillsAndAttributes.value, 75, log=log
            )
        ...
        def TargetPartyMemberPrevious(log=False):
            yield from Yield.Keybinds.PressKeybind(
                ControlAction.ControlAction_TargetPartyMemberPrevious.value, 75, log=log
            )
        ...
        def CommandHero(hero_index: int, log=False):
            if hero_index < 1 or hero_index > 7:
                return
            yield from Yield.Keybinds.PressKeybind(
                ControlAction.ControlAction_CommandHero1.value + (hero_index - 1), 75, log=log
            )
        ...
        def UseSkill(slot: int, log=False):
            if slot < 1 or slot > 8:
                return
            yield from Yield.Keybinds.PressKeybind(
                ControlAction.ControlAction_UseSkill1.value + (slot - 1), 75, log=log
            )
