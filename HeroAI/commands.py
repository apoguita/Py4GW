...
class Command:
    ...
    def __init__(
        ...
        name: str,
        icon: str,
        command_function: Callable[[list[AccountData]], None] | None,
        tooltip: str = "",
        description: str = "",
...
class HeroAICommands:
    ...
    def send_dialog(self, accounts: list[AccountData], dialog_option: int):
        ...
        for account in accounts:
            same_map = (
                own_map_id == account.MapID
                and own_region == account.MapRegion
                and own_district == account.MapDistrict
                and own_language == account.MapLanguage
            )

            if same_map:
                GLOBAL_CACHE.ShMem.SendMessage(
                    sender_email, account.AccountEmail, SharedCommandType.SendDialog, (dialog_option, 0, 0, 0)
                )
    ...
    def __invite_all_command(self, accounts: list[AccountData]):
        ...
        for account in accounts:
            ...
            same_map = (
                Map.GetMapID() == account.MapID
                and Map.GetRegion()[0] == account.MapRegion
                and Map.GetDistrict() == account.MapDistrict
                and Map.GetLanguage()[0] == account.MapLanguage
            )
    ...
    def __talk_and_dialog_with_target_command(self, accounts: list[AccountData]):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.TakeDialogWithTarget, (target_id, 1, 0, 0)
            )
