    ...
    match(ImGui.get_style().Theme):
        case Style.StyleTheme.Guild_Wars:
            ...
            GameTextures.Button.value.draw_in_drawlist(
                ...
                tint=(255, 255, 255, 255) if ImGui.is_mouse_in_rect(
                    button_rect) else (200, 200, 200, 255),
            )
    ...
    if ImGui.begin_with_close("Control Compare", True, PyImGui.WindowFlags.NoFlag):
        if PyImGui.is_rect_visible(50, 50):
            ...
            if ImGui.begin_table("Control Preview", 4, PyImGui.TableFlags.ScrollX):
                ...
                ImGui.text("Combo")
                PyImGui.table_next_column()
                preview.combo = PyImGui.combo("Combo##1", preview.combo, [
                                              "Option 1", "Option 2", "Option 3"])
                PyImGui.table_next_column()
                preview.combo = ImGui.combo("Combo##2", preview.combo, [
                                            "Option 1", "Option 2", "Option 3"])
                PyImGui.table_next_column()
                style.push_style()
                preview.combo = ImGui.combo("Combo##3", preview.combo, [
                                            "Option 1", "Option 2", "Option 3"])
                PyImGui.table_next_column()
                style.pop_style()
                ...
                ImGui.text("Checkbox")
                PyImGui.table_next_column()
                preview.checkbox_2 = PyImGui.checkbox(
                    "##Checkbox 2", preview.checkbox_2)
                PyImGui.same_line(0, 5)
                preview.checkbox = PyImGui.checkbox(
                ...
                PyImGui.table_next_column()
                preview.radio_button = PyImGui.radio_button(
                    "Option 1##Radio Button 1", preview.radio_button, 0)
                preview.radio_button = PyImGui.radio_button(
                    "Option 2##Radio Button 2", preview.radio_button, 1)
                preview.radio_button = PyImGui.radio_button(
                    "Option 3##Radio Button 3", preview.radio_button, 2)
                PyImGui.table_next_column()
                ...
                ImGui.text("Slider")
                PyImGui.table_next_column()
                preview.slider_int = PyImGui.slider_int(
                    "Slider Int", preview.slider_int, 0, 100)
                preview.slider_float = PyImGui.slider_float(
                    "Slider Float", preview.slider_float, 0.0, 100.0)
                PyImGui.table_next_column()
                ...
                ImGui.text("Input")
                PyImGui.table_next_column()
                preview.input_text_value = PyImGui.input_text(
                    "Input Text", preview.input_text_value)
                preview.input_int_value = PyImGui.input_int(
                    "Input Int##2", preview.input_int_value)
                preview.input_int_value = PyImGui.input_int(
                    "Input Int##3", preview.input_int_value, 0, 10000, 0)
                preview.input_float_value = PyImGui.input_float(
                    "Input Float", preview.input_float_value)
...
def DrawWindow():
    ...
    try:
        ...
        if window_module.begin():
            ...
            PyImGui.same_line(0, 5)
            PyImGui.set_cursor_pos_y(PyImGui.get_cursor_pos_y() - 5)
            control_compare = ImGui.checkbox(
                "##show_control_compare", control_compare)
            ImGui.show_tooltip(
                "Show Control Compare window\nThis is used for control development and testing")
                    ...
                    if PyImGui.is_rect_visible(50, 50):
                        ...
                        if ImGui.begin_table("Control Preview", 2, PyImGui.TableFlags.ScrollX):
                            ...
                            if ImGui.primary_button("Primary Button", 0, 25):
                                ConsoleLog(
                                    module_name, "Primary Button clicked")
                            PyImGui.same_line(0, 5)
                            ImGui.primary_button(
                                "Disabled Primary Button", 0, 25, False)
                            PyImGui.table_next_column()
                            ...
                            if ImGui.small_button("Small Button"):
                                ConsoleLog(module_name, "Small Button clicked")
                            PyImGui.same_line(0, 5)
                            ImGui.small_button("Disabled Small Button", False)
                            PyImGui.table_next_column()
                            ...
                            PyImGui.same_line(0, 5)
                            ImGui.icon_button(
                                IconsFontAwesome5.ICON_SYNC, active=False)
                            PyImGui.table_next_column()

                            ImGui.text("Combo")
                            PyImGui.table_next_column()
                            preview.combo = ImGui.combo("Combo", preview.combo, [
                                                        "Option 1", "Option 2", "Option 3"])
                            PyImGui.table_next_column()
                            ...
                            changed, preview.search_value = ImGui.search_field(
                                "Search Field", preview.search_value, "Search...")
                            PyImGui.table_next_column()
                            ...
                            current_style.PlotHistogram.push_color(
                                (219, 150, 251, 255))
                            ImGui.progress_bar(0.25, 0, 20, "25 points")
                            current_style.PlotHistogram.pop_color()
                            PyImGui.table_next_column()
                            ...
                            if ImGui.collapsing_header("Collapsing Header", 0):
                                ImGui.text(
                                    "This is a collapsible header content.")
                            PyImGui.table_next_column()
