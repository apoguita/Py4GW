...
new_favorite = 0
config_module = ImGui.WindowModule(f"Config {module_name}", window_name="Travel##config", window_size=(500, 400), window_flags=PyImGui.WindowFlags.AlwaysAutoResize)
...
def configure():
    ...
    if config_module.begin():
        ...
        if ImGui.begin_tab_bar("##TravelConfigTabs"):                
            if ImGui.begin_tab_item("Favorites"):
                ...
                if widget_config.favorites:
                    if ImGui.begin_table("##FavoritesTable", 4, PyImGui.TableFlags.NoBordersInBody, 0, 0):
            ...
            if ImGui.begin_tab_item("Help"):
                PyImGui.dummy(455, 0)
                ImGui.text("Outpost Travel Configuration")
                ImGui.separator()
                ImGui.text("This widget allows you to travel to outposts.")
                ImGui.bullet_text("search for outposts by name or initials")
                ImGui.bullet_text("travel to the highlighted outpost by pressing Enter")
                ImGui.bullet_text("travel history shows the last 5 outposts you traveled to.")
                ImGui.bullet_text("mark or unmark outposts as favorites with Shift + Left Click")
                ImGui.end_tab_item()
            ImGui.end_tab_bar()
