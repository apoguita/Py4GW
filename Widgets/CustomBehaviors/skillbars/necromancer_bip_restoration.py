...
class NecromancerBipRestorationUtilitySkillBar(CustomBehaviorBaseUtility):
    ...
    def __init__(self, cached_data: CacheData):
        ...
        self.blood_is_power_utility: CustomSkillUtilityBase = BloodIsPowerUtility(
            current_build=in_game_build, score_definition=ScoreStaticDefinition(33)
        )
        self.great_dwarf_weapon_utility: CustomSkillUtilityBase = GreatDwarfWeaponUtility(
            current_build=in_game_build, score_definition=ScoreStaticDefinition(30), mana_required_to_cast=10
        )

        self.breath_of_the_great_dwarf_utility: CustomSkillUtilityBase = BreathOfTheGreatDwarfUtility(
            current_build=in_game_build, score_definition=ScorePerHealthGravityDefinition(9)
        )
        self.spirit_light_utility: CustomSkillUtilityBase = SpiritLightUtility(
            current_build=in_game_build, score_definition=ScorePerHealthGravityDefinition(8)
        )
        self.mend_body_and_soul_utility: CustomSkillUtilityBase = MendBodyAndSoulUtility(
            current_build=in_game_build, score_definition=ScorePerHealthGravityDefinition(7)
        )
        self.soothing_memories_utility: CustomSkillUtilityBase = SoothingMemoriesUtility(
            current_build=in_game_build, score_definition=ScorePerHealthGravityDefinition(6)
        )
        self.protective_was_kaolai_utility: CustomSkillUtilityBase = ProtectiveWasKaolaiUtility(
            current_build=in_game_build, score_definition=ScorePerHealthGravityDefinition(5)
        )

        # optional
        self.flesh_of_my_flesh_utility: CustomSkillUtilityBase = HeroAiUtility(
            skill=CustomSkill("Flesh_of_My_Flesh"), current_build=in_game_build
        )
        self.blood_bond_utility: CustomSkillUtilityBase = BloodBondUtility(
            current_build=in_game_build,
            score_definition=ScorePerAgentQuantityDefinition(lambda enemy_qte: 25 if enemy_qte >= 2 else 0),
