...
class DeadlyParadoxUtility(CustomSkillUtilityBase):
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        has_shadow_form_buff = Routines.Checks.Effects.HasBuff(
            GLOBAL_CACHE.Player.GetAgentID(), self.shadow_form_skill.skill_id
        )
        if not has_shadow_form_buff:
            return self.score_definition.get_score()

        shadow_form_buff_time_remaining = GLOBAL_CACHE.Effects.GetEffectTimeRemaining(
            GLOBAL_CACHE.Player.GetAgentID(), self.shadow_form_skill.skill_id
        )
        if shadow_form_buff_time_remaining <= self.renew_before_shadow_form_expiration_in_milliseconds:
            return self.score_definition.get_score()
