    ...
    def _execute(self, state: BehaviorState) -> Generator[Any, None, BehaviorResult]:
        ...
        if state is BehaviorState.IN_AGGRO:
            ...
            agent_array = AgentArray.Filter.ByCondition(
                agent_array, lambda agent_id: GLOBAL_CACHE.Agent.IsAlive(agent_id)
            )
            agent_array = AgentArray.Filter.ByCondition(
            ...
            if gravity_center is not None:
                if (
                    gravity_center.distance_from_player < Range.Area.value
                ):  # else it doesn't worth moving, we are too far
                    if DEBUG:
                        print("TogetherAsOneUtility: moving to a better place (gravity center).")
