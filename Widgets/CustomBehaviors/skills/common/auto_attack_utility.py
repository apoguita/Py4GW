...
class AutoAttackUtility(CustomSkillUtilityBase):
    def __init__(
        self, current_build: list[CustomSkill], allowed_states: list[BehaviorState] = [BehaviorState.IN_AGGRO]
    ) -> None:
    ...
    def __get_target_agent_id(self) -> int:
        targets = custom_behavior_helpers.Targets.get_all_possible_enemies_ordered_by_priority_raw(
            within_range=Range.Spellcast, sort_key=(TargetingOrder.DISTANCE_ASC, TargetingOrder.HP_ASC)
        )
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        if (
            GLOBAL_CACHE.Agent.IsAttacking(GLOBAL_CACHE.Player.GetAgentID())
            and GLOBAL_CACHE.Player.GetTargetID() == target_agent_id
        ):
            return None
