...
class EbonBattleStandardOfWisdom(CustomSkillUtilityBase):
    ...
    def _get_agent_array(self) -> list[int]:
        ...
        agent_array = AgentArray.Filter.ByCondition(
            agent_array, lambda agent_id: agent_id != GLOBAL_CACHE.Player.GetAgentID()
        )
        agent_array = AgentArray.Filter.ByCondition(
            agent_array, lambda agent_id: GLOBAL_CACHE.Agent.GetProfessionIDs(agent_id)[0] in allowed_classes
        )
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        gravity_center: custom_behavior_helpers.GravityCenter | None = (
            custom_behavior_helpers.Targets.find_optimal_gravity_center(Range.Area, agent_ids=agent_ids)
        )
        ...
        if gravity_center.agent_covered_count >= 2:
            if self.DEBUG:
                print("EbonBattleStandardOfWisdomUtility: moving to a better place (gravity center).")
    ...
    def _execute(self, state: BehaviorState) -> Generator[Any, None, BehaviorResult]:
        ...
        if gravity_center is not None:  # and gravity_center.distance_from_player < Range.Area.value:
            ...
            yield from Routines.Yield.Movement.FollowPath(
                ...
                log=True,
                timeout=4000,
                progress_callback=lambda progress: print(f"EbonBattleStandardOfWisdomUtility: progress: {progress}"),
            )
