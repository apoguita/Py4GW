...
class ArmorOfUnfeelingUtility(CustomSkillUtilityBase):
    ...
    def on_spirit_created(self, message: EventMessage):
        ...
        if spirit_model_id is None:
            return

        if spirit_model_id == SpiritModelID.DISPLACEMENT:
            return
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        if (
            len(self.owned_spirits) > 1
        ):  # we have at least 2 spirits that require armor of unfeeling ; we could wait for only 1 spirit + a duration...
