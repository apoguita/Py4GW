...
class MendBodyAndSoulUtility(CustomSkillUtilityBase):
    ...
    def _get_melee_blind_or_crippled_if_exist(self) -> custom_behavior_helpers.SortableAgentData | None:
        ...
        targets: list[custom_behavior_helpers.SortableAgentData] = (
            custom_behavior_helpers.Targets.get_all_possible_allies_ordered_by_priority_raw(
                ...
                condition=lambda agent_id: GLOBAL_CACHE.Agent.GetProfessionIDs(agent_id)[0] in allowed_classes
                and (CheckForEffect(agent_id, blind_skill_id) or CheckForEffect(agent_id, crippled_skill_id)),
                sort_key=(TargetingOrder.HP_ASC, TargetingOrder.DISTANCE_ASC),
            )
        )
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        blind_or_crippled_melee_target: custom_behavior_helpers.SortableAgentData | None = (
            self._get_melee_blind_or_crippled_if_exist()
        )
    ...
    def _execute(self, state: BehaviorState) -> Generator[Any, None, BehaviorResult]:
        ...
        result = yield from custom_behavior_helpers.Actions.cast_skill_to_target(
            self.custom_skill, target_agent_id=final_target_id
        )
        return result
