...
bot_vars.window_module = ImGui.WindowModule(
    module_name,
    window_name="Apoguita's Vaettir Bot",
    window_size=(300, 300),
...
def HasthingsToSell():
    ...
    if not bot_vars.config_vars.sell_whites:
        items_to_sell = ItemArray.Filter.ByCondition(
            items_to_sell, lambda item_id: Item.Rarity.IsWhite(item_id) == False
        )
    if not bot_vars.config_vars.sell_blues:
        items_to_sell = ItemArray.Filter.ByCondition(
            items_to_sell, lambda item_id: Item.Rarity.IsBlue(item_id) == False
        )
    if not bot_vars.config_vars.sell_purples:
        items_to_sell = ItemArray.Filter.ByCondition(
            items_to_sell, lambda item_id: Item.Rarity.IsPurple(item_id) == False
        )
    if not bot_vars.config_vars.sell_golds:
        items_to_sell = ItemArray.Filter.ByCondition(
            items_to_sell, lambda item_id: Item.Rarity.IsGold(item_id) == False
        )
    if not bot_vars.config_vars.sell_materials:
        items_to_sell = ItemArray.Filter.ByCondition(
            items_to_sell, lambda item_id: Item.Type.IsMaterial(item_id) == False
        )
...
def filter_loot_array(item_array):
    ...
    try:
        ...
        if not bot_vars.config_vars.loot_whites:
            filtered_items = ItemArray.Filter.ByCondition(
                filtered_items, lambda item_id: Item.Rarity.IsWhite(item_id) == False
            )
        if not bot_vars.config_vars.loot_blues:
            filtered_items = ItemArray.Filter.ByCondition(
                filtered_items, lambda item_id: Item.Rarity.IsBlue(item_id) == False
            )
        if not bot_vars.config_vars.loot_purples:
            filtered_items = ItemArray.Filter.ByCondition(
                filtered_items, lambda item_id: Item.Rarity.IsPurple(item_id) == False
            )
        if not bot_vars.config_vars.loot_golds:
            filtered_items = ItemArray.Filter.ByCondition(
                filtered_items, lambda item_id: Item.Rarity.IsGold(item_id) == False
            )
...
def loot_items():
    ...
    if (
        pick_up_item_timer.HasElapsed(1000)
        and filtered_agent_ids
        and Inventory.GetFreeSlotCount() > bot_vars.config_vars.leave_empty_inventory_slots
    ):
...
def finished_looting():
    ...
    if Inventory.GetFreeSlotCount() <= bot_vars.config_vars.leave_empty_inventory_slots or len(filtered_agent_ids) == 0:
        return True
...
def filter_identify_array():
    ...
    unidentified_items = ItemArray.Filter.ByCondition(
        unidentified_items, lambda item_id: Item.Usage.IsIdentified(item_id) == False
    )
    unidentified_items = ItemArray.Filter.ByCondition(
        unidentified_items, lambda item_id: Item.Rarity.IsWhite(item_id) == False
    )

    if not bot_vars.config_vars.id_blues:
        unidentified_items = ItemArray.Filter.ByCondition(
            unidentified_items, lambda item_id: Item.Rarity.IsBlue(item_id) == False
        )
    if not bot_vars.config_vars.id_purples:
        unidentified_items = ItemArray.Filter.ByCondition(
            unidentified_items, lambda item_id: Item.Rarity.IsPurple(item_id) == False
        )
    if not bot_vars.config_vars.id_golds:
        unidentified_items = ItemArray.Filter.ByCondition(
            unidentified_items, lambda item_id: Item.Rarity.IsGold(item_id) == False
        )
...
def filter_salvage_array():
    ...
    if not bot_vars.config_vars.salvage_blues:
        salvageable_items = ItemArray.Filter.ByCondition(
            salvageable_items, lambda item_id: Item.Rarity.IsBlue(item_id) == False
        )
    if not bot_vars.config_vars.salvage_purples:
        salvageable_items = ItemArray.Filter.ByCondition(
            salvageable_items, lambda item_id: Item.Rarity.IsPurple(item_id) == False
        )
    if not bot_vars.config_vars.salvage_golds:
        salvageable_items = ItemArray.Filter.ByCondition(
            salvageable_items, lambda item_id: Item.Rarity.IsGold(item_id) == False
        )
...
def buy_id_kits():
    ...
    merchant_item_list = ItemArray.Filter.ByCondition(
        merchant_item_list, lambda item_id: Item.GetModelID(item_id) == 5899
    )
...
def buy_salvage_kits():
    ...
    merchant_item_list = ItemArray.Filter.ByCondition(
        merchant_item_list, lambda item_id: Item.GetModelID(item_id) == 2992
    )
...
FSM_vars.salvage_fsm.AddState("StartSalvage", execute_fn=lambda: start_alvage(), transition_delay_ms=350)
FSM_vars.salvage_fsm.AddState("ContinueSalvage", execute_fn=lambda: continue_salvage(), transition_delay_ms=350)
FSM_vars.salvage_fsm.AddState("FinishSalvage", execute_fn=lambda: finish_salvage(), transition_delay_ms=300)
FSM_vars.salvage_fsm.AddState("HandleSalvageLoop", execute_fn=lambda: handle_salvage_loop())
...
FSM_vars.sell_to_vendor.AddState(
    name="Target Merchant", execute_fn=lambda: TargetNearestNPC(), transition_delay_ms=1000
)
FSM_vars.sell_to_vendor.AddState(
...
FSM_vars.sell_to_vendor.AddState(
    name="Buy ID Kits", execute_fn=lambda: buy_id_kits(), run_once=False, exit_condition=lambda: buy_id_kits_complete()
)
FSM_vars.sell_to_vendor.AddState(
...
FSM_vars.sell_to_vendor.AddSubroutine(
    name="Salvage Subroutine", sub_fsm=FSM_vars.salvage_fsm, condition_fn=lambda: not has_finished_salvaging()
)
FSM_vars.sell_to_vendor.AddState(name="Reset Salvage Subroutine", execute_fn=lambda: FSM_vars.salvage_fsm.reset())
FSM_vars.sell_to_vendor.AddState(
    name="Sell Materials",
    execute_fn=lambda: SellMaterials(),
    run_once=False,
...
FSM_vars.state_machine.AddState(
    name="Interact NPC", execute_fn=lambda: Routines.Targeting.InteractTarget(), transition_delay_ms=1000
)
FSM_vars.state_machine.AddState(
    name="Take Bounty", execute_fn=lambda: Player.SendDialog(int("0x84", 16)), transition_delay_ms=1000
)
FSM_vars.state_machine.AddState(
...
FSM_vars.state_machine.AddState(name="Reset Salvage Subroutine", execute_fn=lambda: FSM_vars.salvage_fsm.reset())
FSM_vars.state_machine.AddState(
...
FSM_vars.state_machine.AddState(
    name="Waiting for Bjora Return Explorable Map Load",
    exit_condition=lambda: Routines.Transition.IsExplorableLoaded(log_actions=True),
    transition_delay_ms=1000,
)
FSM_vars.state_machine.AddState(
...
FSM_vars.state_machine.AddState(
    name="reset Farming Loop", execute_fn=lambda: reset_farming_loop(), transition_delay_ms=1000
)
FSM_vars.state_machine.AddState(
    name="End State Machine Loop", execute_fn=lambda: reset_farming_loop(), transition_delay_ms=1000
)
...
def CastSkill(skill_id):
    ...
    Py4GW.Console.Log(
        bot_vars.window_module.module_name,
        f"Cast {Skill.GetName(skill_id)}, slot: {SkillBar.GetSlotBySkillID(skill_id)}",
        Py4GW.Console.MessageType.Info,
    )
...
def CastSkill2(skill_slot):
    ...
    Py4GW.Console.Log(
        bot_vars.window_module.module_name,
        f"Cast {Skill.GetName(SkillBar.GetSkillIDBySlot(skill_slot))}, slot: {skill_slot}",
        Py4GW.Console.MessageType.Info,
    )
...
def BjoraRunningSkillbar():
    ...
    if Map.IsMapReady() and not Map.IsMapLoading():
        if Map.IsExplorable() and Map.GetMapID() == 482 and Party.IsPartyLoaded() and CanCast():  # Bjora Marches
            assign_skill_ids()
            ...
            if len(enemy_array) > 0:
                ...
                ) or (HasEnoughEnergy(skillbar.heart_of_shadow) and FSM_vars.non_movement_timer.HasElapsed(3000)):
...
def FarmingSkillbar():
    ...
    if Map.IsMapReady() and not Map.IsMapLoading():
        if Map.IsExplorable() and Map.GetMapID() == 546 and Party.IsPartyLoaded() and CanCast():  # Jaga Moraine
            assign_skill_ids()
            ...
            if FSM_vars.in_killing_routine:
                ...
                not_hexed_array = AgentArray.Filter.ByDistance(
                    not_hexed_array, (player_x, player_y), area_distance.Area
                )
                        ...
                        if PyImGui.begin_tab_bar("StatsTabBar"):
                            if PyImGui.begin_tab_item("Run"):
                                ...
                                data = [
                                    (
                                        "Total Run Time",
                                        f"{bot_vars.window_statistics.global_timer.FormatElapsedTime("hh:mm:ss:ms")}",
                                    ),
                                    (
                                        "Current Run Time",
                                        f"{bot_vars.window_statistics.lap_timer.FormatElapsedTime("hh:mm:ss:ms")}",
                                    ),
                    ...
                    if PyImGui.begin_tab_item("Config"):
                        if PyImGui.begin_tab_bar("ConfigTabBar"):
                            if PyImGui.begin_tab_item("Looting"):
                                ...
                                bot_vars.config_vars.loot_whites = PyImGui.checkbox(
                                    "Loot Whites", bot_vars.config_vars.loot_whites
                                )
                                bot_vars.config_vars.loot_blues = PyImGui.checkbox(
                                    "Loot Blues", bot_vars.config_vars.loot_blues
                                )
                                bot_vars.config_vars.loot_purples = PyImGui.checkbox(
                                    "Loot Purples", bot_vars.config_vars.loot_purples
                                )
                                bot_vars.config_vars.loot_golds = PyImGui.checkbox(
                                    "Loot Golds", bot_vars.config_vars.loot_golds
                                )
                                bot_vars.config_vars.loot_tomes = PyImGui.checkbox(
                                    "Loot Tomes", bot_vars.config_vars.loot_tomes
                                )
                                bot_vars.config_vars.loot_dyes = PyImGui.checkbox(
                                    "Loot Dyes", bot_vars.config_vars.loot_dyes
                                )
                                bot_vars.config_vars.loot_glacial_stones = PyImGui.checkbox(
                                    "Loot Glacial Stones", bot_vars.config_vars.loot_glacial_stones
                                )
                                bot_vars.config_vars.loot_event_items = PyImGui.checkbox(
                                    "Loot Event Items", bot_vars.config_vars.loot_event_items
                                )
                            ...
                            if PyImGui.begin_tab_item("ID & Salvage"):
                                ...
                                bot_vars.config_vars.id_blues = PyImGui.checkbox(
                                    "ID Blues", bot_vars.config_vars.id_blues
                                )
                                bot_vars.config_vars.id_purples = PyImGui.checkbox(
                                    "ID Purples", bot_vars.config_vars.id_purples
                                )
                                bot_vars.config_vars.id_golds = PyImGui.checkbox(
                                    "ID Golds", bot_vars.config_vars.id_golds
                                )
                                PyImGui.separator()
                                bot_vars.config_vars.salvage_whites = PyImGui.checkbox(
                                    "Salvage Whites", bot_vars.config_vars.salvage_whites
                                )
                                ...
                                bot_vars.config_vars.salvage_glacial_stones = PyImGui.checkbox(
                                    "Salvage Glacial Stones", bot_vars.config_vars.salvage_glacial_stones
                                )
                            ...
                            if PyImGui.begin_tab_item("Selling"):
                                ...
                                bot_vars.config_vars.sell_whites = PyImGui.checkbox(
                                    "Sell Whites", bot_vars.config_vars.sell_whites
                                )
                                bot_vars.config_vars.sell_blues = PyImGui.checkbox(
                                    "Sell Blues", bot_vars.config_vars.sell_blues
                                )
                                bot_vars.config_vars.sell_purples = PyImGui.checkbox(
                                    "Sell Purples", bot_vars.config_vars.sell_purples
                                )
                                bot_vars.config_vars.sell_golds = PyImGui.checkbox(
                                    "Sell Golds", bot_vars.config_vars.sell_golds
                                )
                                PyImGui.separator()
                                bot_vars.config_vars.sell_materials = PyImGui.checkbox(
                                    "Sell Materials", bot_vars.config_vars.sell_materials
                                )
                                bot_vars.config_vars.sell_wood = PyImGui.checkbox(
                                    "Sell Wood", bot_vars.config_vars.sell_wood and bot_vars.config_vars.sell_materials
                                )
                                bot_vars.config_vars.sell_iron = PyImGui.checkbox(
                                    "Sell Iron", bot_vars.config_vars.sell_iron and bot_vars.config_vars.sell_materials
                                )
                                bot_vars.config_vars.sell_dust = PyImGui.checkbox(
                                    "Sell Dust", bot_vars.config_vars.sell_dust and bot_vars.config_vars.sell_materials
                                )
                                bot_vars.config_vars.sell_cloth = PyImGui.checkbox(
                                    "Sell Cloth",
                                    bot_vars.config_vars.sell_cloth and bot_vars.config_vars.sell_materials,
                                )
...
def Handle_Stuck():
    ...
    if FSM_vars.stuck_count > 10:
        Py4GW.Console.Log(
            bot_vars.window_module.module_name,
            f"Player is stuck, cannot recover, restarting.",
            Py4GW.Console.MessageType.Error,
        )
    ...
    if (
        not Agent.IsMoving(Player.GetAgentID()) and not FSM_vars.in_waiting_routine
        ...
        if FSM_vars.non_movement_timer.HasElapsed(3000):
            ...
            Py4GW.Console.Log(
                bot_vars.window_module.module_name,
                f"Player is stuck, attempting to recover",
