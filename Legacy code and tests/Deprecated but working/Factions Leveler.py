...
class FSM_Config:
    ...
    def initialize(self):
        ...
        self.FSM.AddYieldRoutineStep(
            name="Wait shing jea Monastery to Load 001",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Shing Jea Monastery")),
        )
        self.FSM.AddYieldRoutineStep(name="Exit to Courtyard 001", coroutine_fn=self.ExitToCourtyard001)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Linnok Courtyard Map Load 001",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Linnok Courtyard")),
        )
        self.FSM.AddYieldRoutineStep(name="Unlock Secondary and Exit", coroutine_fn=self.UnlockSecondaryAndExit)
        self.FSM.AddYieldRoutineStep(
            name="Wait shing jea Monastery to Load 002",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Shing Jea Monastery")),
        )
        ...
        self.FSM.AddYieldRoutineStep(
            name="Wait for Sunqua Vale Map Load 001",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Sunqua Vale")),
        )
        self.FSM.AddYieldRoutineStep(name="Travel to Minister Cho", coroutine_fn=self.TravelToMinisterCho)
        minister_cho_map_id = 214
        self.FSM.AddYieldRoutineStep(
            name="Wait for Minister Cho Map Load", coroutine_fn=lambda: self.WaitforMapLoad(minister_cho_map_id)
        )
        self.FSM.AddYieldRoutineStep(
            name="Prepare for Minister Cho Mission", coroutine_fn=self.PrepareForMinisterChoMission
        )
        self.FSM.AddYieldRoutineStep(
            name="Wait for Minister Cho Mission Load", coroutine_fn=lambda: self.WaitforMapLoad(minister_cho_map_id)
        )
        self.FSM.AddYieldRoutineStep(name="Minister Cho Mission", coroutine_fn=self.MinisterChoMission)
        self.FSM.AddYieldRoutineStep(
            name="Take Warning the Tengu and Exit", coroutine_fn=self.TakeWarningTheTenguandExit
        )
        self.FSM.AddYieldRoutineStep(
            name="Wait for Kinya Province Map Load",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Kinya Province")),
        )  # 236
        self.FSM.AddYieldRoutineStep(name="Warning the Tengu", coroutine_fn=self.WarningTheTengu)
        self.FSM.AddYieldRoutineStep(
            name="Wait shing jea Monastery to Load 003",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Shing Jea Monastery")),
        )
        self.FSM.AddYieldRoutineStep(name="Exit Monastery Overlook 002", coroutine_fn=self.ExitMonasteryOverlook002)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Sunqua Vale Map Load 002",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Sunqua Vale")),
        )
        self.FSM.AddYieldRoutineStep(name="Travel to Tsumei Village", coroutine_fn=self.TravelTsumeiVillage)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Tsumei Village Map Load",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Tsumei Village")),
        )
        self.FSM.AddYieldRoutineStep(name="Exit Tsumei Village", coroutine_fn=self.ExitTsumeiVillage)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Panjiang Peninsula Map Load",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Panjiang Peninsula")),
        )
        self.FSM.AddYieldRoutineStep(name="The Threat Grows", coroutine_fn=self.TheThreatGrows)
        self.FSM.AddYieldRoutineStep(
            name="Wait shing jea Monastery to Load 004",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Shing Jea Monastery")),
        )
        self.FSM.AddYieldRoutineStep(name="Exit to Courtyard 002", coroutine_fn=self.ExitToCourtyard002)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Linnok Courtyard Map Load 002",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Linnok Courtyard")),
        )
        self.FSM.AddYieldRoutineStep(
            name="Finish quest and advance to Saoshang Trail", coroutine_fn=self.FinishQuestsAndAdvanceToSaoshangTrail
        )
        saoshang_trail_map_id = 313
        self.FSM.AddYieldRoutineStep(
            name="Wait for Saoshang Trail Map Load", coroutine_fn=lambda: self.WaitforMapLoad(saoshang_trail_map_id)
        )
        self.FSM.AddYieldRoutineStep(name="Traverse Saoshang Trail", coroutine_fn=self.TraverseSaoshangTrail)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Seitung Harbor Map Load 001",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Seitung Harbor")),
        )
        self.FSM.AddYieldRoutineStep(
            name="Take Reward and Exit Seitung Harbor", coroutine_fn=self.TakeRewardAndExitSeitungHarbor
        )
        self.FSM.AddYieldRoutineStep(
            name="Wait for Jaya Bluffs Map Load",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Jaya Bluffs")),
        )
        self.FSM.AddYieldRoutineStep(name="Go to Zen Daijun 001", coroutine_fn=self.GoToZenDaijunPart001)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Haiju Lagoon Map Load",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Haiju Lagoon")),
        )
        self.FSM.AddYieldRoutineStep(name="Go to Zen Daijun 002", coroutine_fn=self.GoToZenDaijunPart002)
        zen_daijun_map_id = 213
        self.FSM.AddYieldRoutineStep(
            name="Wait for Zen Daijun Map Load", coroutine_fn=lambda: self.WaitforMapLoad(zen_daijun_map_id)
        )
        self.FSM.AddYieldRoutineStep(
            name="Prepare for Zen Daijun Mission", coroutine_fn=self.PrepareForZenDaijunMission
        )
        self.FSM.AddYieldRoutineStep(name="Zen Daijun Mission", coroutine_fn=self.ZenDaijunMission)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Seitung Harbor Map Load 002",
            coroutine_fn=lambda: self.WaitforMapLoad(Map.GetMapIDByName("Seitung Harbor")),
        )
        ...
        if self.use_cupcakes:
            ...
            if cupcakes_needed > 0 and cupcake_in_storage > 0:
                ...
                if not items_withdrawn:
                    ...
                    if not items_withdrawn:
                        Py4GW.Console.Log(
                            MODULE_NAME, "Failed to withdraw cupcakes from storage.", Py4GW.Console.MessageType.Error
                        )
        ...
        if self.use_honeycombs:
            ...
            if honey_needed > 0 and honey_in_storage > 0:
                ...
                if not items_withdrawn:
                    ...
                    if not items_withdrawn:
                        Py4GW.Console.Log(
                            MODULE_NAME, "Failed to withdraw honeycombs from storage.", Py4GW.Console.MessageType.Error
                        )
    ...
    def AutoCombat(self):
        ...
        while True:
            if not (
                ...
                and not self.combat_handler.InCastingRoutine()
            ):
    ...
    def interact_with_agent(
        self, coords: Tuple[float, float], dialog_id: Optional[int] = None
    ...
    def CraftWeapons(self):
        ...
        if profession in MELEE_CLASSES:
            ...
            if iron_in_storage < 5:
                Py4GW.Console.Log(
                    MODULE_NAME, "Not enough Iron Ingots (5) to craft weapons.", Py4GW.Console.MessageType.Error
                )
                yield from self._stop_execution()
                return
            ...
            if not items_withdrawn:
                Py4GW.Console.Log(
                    MODULE_NAME, "Failed to withdraw (5) Iron Ingots from storage.", Py4GW.Console.MessageType.Error
                )
                yield from self._stop_execution()
                return
            ...
            if crafted_weapon:
                ...
                Py4GW.Console.Log(
                    MODULE_NAME, "Crafted weapon not found in inventory.", Py4GW.Console.MessageType.Error
                )
                yield from self._stop_execution()
                return
        else:
    ...
    def WarningTheTengu(self):
        first_half_path: List[Tuple[float, float]] = [
            (14621, 14767),
            (14018, 15121),
            (13218, 15462),
            (12393, 15581),
        ...
        try:
            ...
            second_half_path: List[Tuple[float, float]] = [
                (687, 12331),
                (-115, 11996),
                (-920, 11681),
                (-1782, 11613),
    ...
    def TravelTsumeiVillage(self):
        path_to_tsumei: List[Tuple[float, float]] = [
            (20323, -8150),
            (19816, -8562),
            (19191, -9163),
            (18379, -9169),
    ...
    def TheThreatGrows(self):
        path_to_quest: List[Tuple[float, float]] = [
            (9687, 16174),
            (9884, 15533),
            (10191, 14723),
            (10528, 13927),
        ...
        try:
            ...
            while True:
                ...
                if (not Routines.Checks.Agents.InDanger(aggro_area=Range.Spellcast)) and Agent.HasQuest(
                    sister_tai_agent_id
                ):
                    break
                yield from Routines.Yield.wait(1000)
    ...
    def ExitToCourtyard002(self):
        ...
        path_to_courtyard: List[Tuple[float, float]] = [
            (-8861, 8508),
            (-8350, 8960),
            (-7558, 9307),
            (-6718, 9488),
    ...
    def FinishQuestsAndAdvanceToSaoshangTrail(self):
        path_to_togo: List[Tuple[float, float]] = [
            (-3281, 9442),
            (-2673, 9447),
            (-1790, 9441),
            (-904, 9434),
    ...
    def TraverseSaoshangTrail(self):
        ...
        path_to_saoshang: List[Tuple[float, float]] = [
            (1185, 10837),
            (1574, 10532),
            (2364, 10186),
            (2918, 9910),
    ...
    def TakeRewardAndExitSeitungHarbor(self):
        ...
        path_to_exit: List[Tuple[float, float]] = [
            (16404, 12067),
            (16835, 12615),
            (17477, 13201),
            (18250, 13437),
    ...
    def ZenDaijunMission(self):
        ...
        try:
            path_to_mission001: List[Tuple[float, float]] = [
                (16209, 11436),
                (15963, 11216),
                (15963, 11216),
                (15678, 10945),
            ...
            path_to_mission002: List[Tuple[float, float]] = [
                (11778, 11357),
                (11578, 10636),
                (10668, 10063),
                (9674, 9621),
            ...
            path_to_mission003: List[Tuple[float, float]] = [
                (4892, 1523),
                (5648, 1911),
                (5941, 1166),
                (5605, 132),
            ...
            path_to_mission004: List[Tuple[float, float]] = [
                (-8625, -742),
                (-8298, -1270),
                (-8198, -1147),
                (-7851, -1458),
...
def ShowMainWindow():
    ...
    if PyImGui.begin(MODULE_NAME, PyImGui.WindowFlags.AlwaysAutoResize):
        ...
        main_FSM.amount_of_honeycombs = PyImGui.slider_int(
            "Amount of Honeycombs", main_FSM.amount_of_honeycombs, 1, 100
        )
