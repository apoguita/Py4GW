...
def test_build_tracker_drop_message_parses_meta_and_params():
    meta = build_drop_meta("85e699300008", "deadbeef", "03:47 PM", sender_session_id=0x0011)
    ...
    assert msg.sender_session_id == 0x0011
...
def test_handle_inventory_stats_response_branch_parses_and_upserts():
    ...
    handled = handle_inventory_stats_response_branch(
        ...
        extra_data_list=["TrackerInvStatRes", "Player Three", "2", ""],
...
def test_parse_tracker_name_chunk_reads_optional_event_id_from_meta():
    chunk = parse_tracker_name_chunk(
        extra_data_list=["TrackerNameV2", "sig123", "Holy ", "1/2|85e699300008"],
        to_text_fn=lambda v: str(v),
        decode_chunk_meta_fn=lambda text: tuple(int(p) for p in str(text).split("|", 1)[0].split("/", 1)),
    )
    assert chunk is not None
    assert chunk.event_id == "85e699300008"
...
def test_handle_tracker_drop_branch_keeps_truncated_name_when_unresolved():
    ...
    assert drop_msg is not None
    assert drop_msg.item_name == long_name
...
def test_handle_tracker_drop_branch_ignores_mismatched_resolved_name():
    ...
    assert drop_msg is not None
    # Keep raw/truncated name instead of applying mismatched resolved name.
    assert drop_msg.item_name == long_name
