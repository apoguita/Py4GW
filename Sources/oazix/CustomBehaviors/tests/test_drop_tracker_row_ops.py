...
def test_set_runtime_fields_extend_short_rows():
    row = ["2026-02-22 15:47:00", "Leader", "248", "Ice Caves", "Player Three", "Holy Staff", "1", "White"]
...
def test_update_rows_item_stats_by_event_and_player():
    ...
    rows = [row_a, row_b]
    updated = update_rows_item_stats_by_event_and_player(rows, "ev-a", "Player Three", "stats-a")
    assert updated == 1
...
def test_update_rows_item_stats_by_event_and_player_empty_event_returns_zero():
    ...
    assert update_rows_item_stats_by_event_and_player(rows, "", "Player Three", "ignored") == 0
...
def test_update_rows_item_stats_by_event_and_player_skips_invalid_rows():
    ...
    updated = update_rows_item_stats_by_event_and_player(rows, "ev-skip", "Player Three", "stats-skip")
    assert updated == 1
