...
def IsSkillBarLoaded():
    ...
    if primary_profession == "Assassin" and secondary_profession != "Mesmer":
        ...
        ConsoleLog(
            IsSkillBarLoaded, f"This bot requires A/Me to work, halting.", Py4GW.Console.MessageType.Error, log=True
        )
        return False
    ...
    ConsoleLog(
        MODULE_NAME, f"SkillBar Loaded.", Py4GW.Console.MessageType.Info, log=bot_variables.config.log_to_console
    )
    return True
...
def InventoryHandler(log_to_console=False):
    if NeedsToHandleInventory():
        ...
        if log_to_console:
            ConsoleLog(
                MODULE_NAME, f"Salvaging {items_to_salvage} items.", Py4GW.Console.MessageType.Info, log=log_to_console
            )
...
def RunBotSequentialLogic():
    ...
    while True:
        ...
        if not bot_variables.config.reset_from_jaga_moraine:
            ...
            ConsoleLog(
                MODULE_NAME, "Finished traversing bjora marches", Py4GW.Console.MessageType.Info, log=log_to_console
            )
        ...
        ConsoleLog(
            MODULE_NAME, "Finished waiting for left aggro ball", Py4GW.Console.MessageType.Info, log=log_to_console
        )
        bot_variables.config.pause_stuck_routine = False
        ...
        yield from Routines.Yield.wait(15000)
        ConsoleLog(
            MODULE_NAME, "Finished waiting for right aggro ball", Py4GW.Console.MessageType.Info, log=log_to_console
        )
        bot_variables.config.pause_stuck_routine = False
        ...
        if target:
            ...
            if (
                ...
                if (
                    ...
                    ConsoleLog(
                        MODULE_NAME, "Casting Echoed Wastrel.", Py4GW.Console.MessageType.Info, log=log_to_console
                    )
                    return
        target = GetNotHexedEnemy()
        if target:
