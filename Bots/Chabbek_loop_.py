...
def ApproachJahdugargate(bot: Botting):
    ...
    bot.Move.XY(7696, 5279)
    bot.Move.XY(5626, 1542)
    bot.Move.XY(3088, 660)
...
def TravelToBeach(bot: Botting) -> None:
...
from Py4GWCoreLib import Map  # important
...
import random
from Py4GW import Console
from Py4GWCoreLib import ConsoleLog, Routines, Map
...
def TravelToChabbek(bot: Botting) -> None:
    bot.States.AddHeader("Travel to Chabbek")

    def _state():
        # 7=EU, 8=EU+INT, 11=ALL (incl. Asia)
        yield from RndTravelState(544, use_districts=8)

    bot.States.AddCustomState(_state, "RndTravel -> Chabbek")
